  ...
  for(sim in 1:nSims){
    ...
    for(yy in 1:length(Years)){
      ...
      for(pp in 1:NP){
        ...
        for(ss in 1:NS){
          if(Stocks[ss] %in% ss_mature){
            ...
            Shape1m <- Means^2*(((1-Means)/(SDs^2))-(1/Means))
            Shape2m <- Shape1m*(1/Means-1)
            if (max(Ages)==6){MatRates<-NULL
                              MatRates[1]<-0}
            #Need to find a way to make this changable, if you do all ages, it works, but produces warnings
            if (max(Ages)==5){MatRates <- rbeta(length(Shape1m[1:3]),Shape1m[1:3], Shape2m[1:3])}
            if (max(Ages)==6){MatRates[2:4] <- rbeta(length(Shape1m[2:4]),Shape1m[2:4], Shape2m[2:4])}
            # Age 5 maturation rates have to be added after since a beta distribution with a SD of 0 produces NAs
            if (max(Ages)==5){MatRates[4]<-1}
            if (max(Ages)==6){MatRates[5]<-1}
      ...
      for(ss in 1:NS){
        ...
        if(Opts$BigBar != 0){ # remove catch from here???? half it???
  ...
  for(ss in 1:NS) {
    for(aa in 1:length(Ages)){
      ...
      #Dist_Vec <- RDist[which(RDist$StockID==as.character(Stocks[ss]) & RDist$Period==pp & RDist$Age == Ages[aa]),
       #                 which(names(RDist) %in% Region_Tab$Region_Name)]
      # use this dist vector to distribute abund of this stock and age
      #if (Abund[aa] < 0) {Abund[aa] == 0}
      #print(Abund[aa])
      #print(yy)
      #N_out[,ss,aa] <- rmultinom(n=1, size= Abund[aa], prob=Dist_Vec) # Abund[ss, aa] if more than one stock
